...
#define CRNLIB_PIXEL_FMT_FOURCC(a, b, c, d) \
	((a) | ((b) << 8U) | ((c) << 16U) | ((d) << 24U))
...
enum pixel_format {
	...
	PIXEL_FMT_DXT5A = CRNLIB_PIXEL_FMT_FOURCC(
		'A',
		'T',
		'I',
		'1'), // ATI1N,
			  // http://developer.amd.com/media/gpu_assets/Radeon_X1x00_Programming_Guide.pdf

	// Non-standard, crnlib-specific pixel formats (some of these are supported by ATI's
	// Compressonator)
...
struct DDPIXELFORMAT {
	...
	crn_uint32 dwFlags;
	crn_uint32 dwFourCC;
	crn_uint32 dwRGBBitCount; // ATI compressonator and crnlib will place a FOURCC code
							  // here for swizzled/cooked DXTn formats
