...
// (most of them supported by ATI's Compressonator), along with some ATI/X360 GPU
// specific formats.
enum crn_format {
	cCRNFmtInvalid = -1,
...
enum crn_dxt_compressor_type {
	cCRNDXTCompressorCRN, // Use crnlib's ETC1 or DXTc block compressor (default,
						  // highest quality, comparable or better than ati_compress or
						  // squish, and crnlib's ETC1 is a lot fasterw with similiar
						  // quality to Erricson's)
	cCRNDXTCompressorCRNF, // Use crnlib's "fast" DXTc block compressor
	cCRNDXTCompressorRYG, // Use RYG's DXTc block compressor (low quality, but very
						  // fast)
...
typedef crn_bool (*crn_progress_callback_func)(
	crn_uint32 phase_index,
...
struct crn_comp_params {
	...
	inline void clear() {
		...
		m_flags = cCRNCompFlagPerceptual | cCRNCompFlagHierarchical |
...
#undef CRNLIB_COMP
	...
	inline bool check() const {
		if ((m_file_type > cCRNFileTypeDDS) ||
			(((int)m_quality_level < (int)cCRNMinQualityLevel) ||
			 ((int)m_quality_level > (int)cCRNMaxQualityLevel)) ||
			(m_dxt1a_alpha_threshold > 255) || ((m_faces != 1) && (m_faces != 6)) ||
			...
			((m_crn_color_endpoint_palette_size) &&
			 ((m_crn_color_endpoint_palette_size < cCRNMinPaletteSize) ||
			  (m_crn_color_endpoint_palette_size > cCRNMaxPaletteSize))) ||
			((m_crn_color_selector_palette_size) &&
			 ((m_crn_color_selector_palette_size < cCRNMinPaletteSize) ||
			  (m_crn_color_selector_palette_size > cCRNMaxPaletteSize))) ||
			((m_crn_alpha_endpoint_palette_size) &&
			 ((m_crn_alpha_endpoint_palette_size < cCRNMinPaletteSize) ||
			  (m_crn_alpha_endpoint_palette_size > cCRNMaxPaletteSize))) ||
			((m_crn_alpha_selector_palette_size) &&
			 ((m_crn_alpha_selector_palette_size < cCRNMinPaletteSize) ||
			  (m_crn_alpha_selector_palette_size > cCRNMaxPaletteSize))) ||
			(m_alpha_component > 3) || (m_num_helper_threads > cCRNMaxHelperThreads) ||
			(m_dxt_quality > cCRNDXTQualityUber) ||
	...
	// Target bitrate - if non-zero, the compressor will use an interpolative search to
	// find the highest quality level that is <= the target bitrate. If it fails to find
	// a bitrate high enough, it'll try disabling adaptive block sizes
	// (cCRNCompFlagHierarchical flag) and redo the search. This process can be pretty
	// slow.
	float m_target_bitrate;

	// Desired quality level.
	// Currently, CRN and DDS quality levels are not compatible with eachother from an
	...
	crn_uint32
		m_crn_color_endpoint_palette_size; // [cCRNMinPaletteSize,cCRNMaxPaletteSize]
	crn_uint32
		m_crn_color_selector_palette_size; // [cCRNMinPaletteSize,cCRNMaxPaletteSize]

	crn_uint32
		m_crn_alpha_endpoint_palette_size; // [cCRNMinPaletteSize,cCRNMaxPaletteSize]
	crn_uint32
		m_crn_alpha_selector_palette_size; // [cCRNMinPaletteSize,cCRNMaxPaletteSize]

	// Number of helper threads to create during compression. 0=no threading.
	crn_uint32 m_num_helper_threads;

	// CRN userdata0 and userdata1 members, which are written directly to the header of
	// the output file.
...
enum crn_mip_mode {
	cCRNMipModeUseSourceOrGenerateMips, // Use source texture's mipmaps if it has any,
										// otherwise generate new mipmaps
	cCRNMipModeUseSourceMips, // Use source texture's mipmaps if it has any, otherwise
							  // the output has no mipmaps
...
// Function to set an optional user provided memory allocation/reallocation/msize
// routines. By default, crnlib just uses malloc(), free(), etc. for all allocations.
typedef void *(*crn_realloc_func)(
	void *p, size_t size, size_t *pActual_size, bool movable, void *pUser_data);
typedef size_t (*crn_msize_func)(void *p, void *pUser_data);
void crn_set_memory_callbacks(
	crn_realloc_func pRealloc, crn_msize_func pMSize, void *pUser_data);

// Frees memory blocks allocated by crn_compress(), crn_decompress_crn_to_dds(), or
// crn_decompress_dds_to_images().
void crn_free_block(void *pBlock);

// Compresses a 32-bit/pixel texture to either: a regular DX9 DDS file, a "clustered"
// (or reduced entropy) DX9 DDS file, or a CRN file in memory. Input parameters:
//  comp_params is the compression parameters struct, defined above.
//  compressed_size will be set to the size of the returned memory block containing the
//  output file. The returned block must be freed by calling crn_free_block().
//  *pActual_quality_level will be set to the actual quality level used to compress the
//  image. May be NULL. *pActual_bitrate will be set to the output file's effective
//  bitrate, possibly taking into account LZMA compression. May be NULL.
// Return value:
//  The compressed file data, or NULL on failure.
//  compressed_size will be set to the size of the returned memory buffer.
// Notes:
//  A "regular" DDS file is compressed using normal DXTn compression at the specified
...
//  memory), grayscale/alpha (YYYA in memory), 24-bit (RGBX in memory), or 32-bit (RGBA)
//  colors (where "X"=don't care). RGB color data is generally assumed to be in the sRGB
//  colorspace. If not, be sure to clear the "cCRNCompFlagPerceptual" in the
//  crn_comp_params struct!
void *crn_compress(
	const crn_comp_params &comp_params,
	crn_uint32 &compressed_size,
	crn_uint32 *pActual_quality_level = NULL,
	float *pActual_bitrate = NULL);

// Like the above function, except this function can also do things like generate
// mipmaps, and resize or crop the input texture before compression. The actual
// operations performed are controlled by the crn_mipmap_params struct members. Be sure
// to set the "m_gamma_filtering" member of crn_mipmap_params to false if the input
// texture is not sRGB.
void *crn_compress(
	const crn_comp_params &comp_params,
...
// Transcodes an entire CRN file to DDS using the crn_decomp.h header file library to do
// most of the heavy lifting. The output DDS file's format is guaranteed to be one of
// the DXTn formats in the crn_format enum. This is a fast operation, because the CRN
// format is explicitly designed to be efficiently transcodable to DXTn. For more
// control over decompression, see the lower-level helper functions in crn_decomp.h,
// which do not depend at all on crnlib.
void *crn_decompress_crn_to_dds(const void *pCRN_file_data, crn_uint32 &file_size);

// Decompresses an entire DDS file in any supported format to uncompressed 32-bit/pixel
// image(s). See the crnlib::pixel_format enum in inc/dds_defs.h for a list of the
// supported DDS formats. You are responsible for freeing each image block, either by
// calling crn_free_all_images() or manually calling crn_free_block() on each image
// pointer.
...
// crnlib's DXTn endpoint optimizer actually supports any number of source pixels (i.e.
// from 1 to thousands, not just 16), but for simplicity this API only supports 4x4
// texel blocks.
typedef void *crn_block_compressor_context_t;

// Create a DXTn block compressor.
// This function only supports the basic/nonswizzled "fundamental" formats: DXT1, DXT3,
// DXT5, DXT5A, DXN_XY and DXN_YX. Avoid calling this multiple times if you intend on
// compressing many blocks, because it allocates some memory.
crn_block_compressor_context_t
crn_create_block_compressor(const crn_comp_params &params);
...
// pPixels should be an array of 16 crn_uint32's. Each crn_uint32 must be r,g,b,a (r is
// always first) in memory.
void crn_compress_block(
	crn_block_compressor_context_t pContext,
	const crn_uint32 *pPixels,
	void *pDst_block);
...
// pDst_pixel should be an array of 16 crn_uint32's. Each uint32 will be r,g,b,a (r is
// always first) in memory. crn_fmt should be one of the "fundamental" formats: DXT1,
// DXT3, DXT5, DXT5A, DXN_XY and DXN_YX. The various swizzled DXT5 formats (such as
// cCRNFmtDXT5_xGBR, etc.) will be unpacked as if they where plain DXT5. Returns false
// if the crn_fmt is invalid.
bool crn_decompress_block(
	const void *pSrc_block, crn_uint32 *pDst_pixels, crn_format crn_fmt);
